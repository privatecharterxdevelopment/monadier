...
const ProTradeLeaderboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [top, live] = await Promise.all([
        fetchBotPublicLeaderboard(15),
        fetchBotPublicLiveWins(8),
      ]);
      if (cancelled) return;
      setTopTrades(top);
      setLiveTrades(live);
  ...
  return (
    <ProTradePageShell className="hl-leaderboard-page">
      ...
      <section className="hl-leaderboard-panel">
        <div className="hl-leaderboard-panel__head">
          <h2>Top wins</h2>
          <span>On-chain · HypurrScan</span>
        </div>
      ...
      <section className="hl-leaderboard-panel hl-leaderboard-panel--live">
        <div className="hl-leaderboard-panel__head">
