...
type NotifyKey = 'trade_close_email_enabled' | 'community_mention_email_enabled';
...
const ProfileNotificationSettings: React.FC = () => {
  ...
  const onToggle = useCallback(
    async (key: NotifyKey) => {
      ...
      const current = key === 'trade_close_email_enabled' ? tradeEnabled : communityEnabled;
      const next = !current;
      setBusyKey(key);
      setError(null);
      if (key === 'trade_close_email_enabled') setTradeEnabled(next);
      else setCommunityEnabled(next);
      try {
    ...
    [user?.id, busyKey, tradeEnabled, communityEnabled, refreshProfile, t]
  );
