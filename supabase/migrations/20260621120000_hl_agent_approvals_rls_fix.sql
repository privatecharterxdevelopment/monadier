-- Fix RLS on hl_agent_approvals: upsert via SECURITY DEFINER after wallet is linked.
...
BEGIN
  ...
  -- Link wallet to the signed-in user so SELECT policies work.
...
-- Upsert needs INSERT + UPDATE WITH CHECK; tighten UPDATE policy.
