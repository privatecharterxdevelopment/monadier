-- Admin: align bot counts with Railway bot-service; per-wallet fee owed/paid; runnable blockers.
  ...
  v_result := jsonb_build_object(
      ...
      SELECT jsonb_build_object(
        ...
        'hl_bots_runnable', (
          ...
          LEFT JOIN public.wallet_platform_fee_state wps
            ON lower(wps.wallet_address) = lower(vs.wallet_address)
          WHERE coalesce(vs.execution_venue, 'hyperliquid') = 'hyperliquid'
            ...
            AND NOT (
              ...
              AND coalesce(wps.success_win_count, 0) >= v_fee_win_block
            )
            AND NOT EXISTS (
          ...
          trim(both ' · ' FROM concat_ws(
            ...
            CASE
              WHEN coalesce(fee.fees_accrued_usd, 0) > 0.000001
                AND coalesce(wps.success_win_count, 0) >= v_fee_win_block
                THEN 'fees due (' || coalesce(wps.success_win_count, 0)::text || '/20 wins)'
            END,
            CASE
              WHEN EXISTS (
                SELECT 1 FROM public.platform_fee_waivers w
    ...
    'wallet_fees', coalesce((
      ...
      FROM (
        SELECT
          ...
          coalesce(wps.success_win_count, 0)::int AS fee_win_count,
          greatest(0, v_fee_win_block - coalesce(wps.success_win_count, 0))::int AS wins_until_fee,
          (
            coalesce(fee.fees_accrued_usd, 0) > 0.000001
            AND coalesce(wps.success_win_count, 0) >= v_fee_win_block
          ) AS fee_opens_blocked,
    ...
    'users', coalesce((
      ...
      FROM (
        ...
        LEFT JOIN public.wallet_platform_fee_state wps
          ON lower(wps.wallet_address) = lower(coalesce(nullif(trim(p.wallet_address), ''), vw.w))
