-- Public bot leaderboard: sanitized rows via SECURITY DEFINER (bypasses trade_history user RLS).
-- Wallets are public on Hyperliquid; only masked labels + HL fields are exposed.
...
AS $$
BEGIN
  IF lower(coalesce(p_sort, 'top')) = 'recent' THEN
    RETURN QUERY
    SELECT
      th.id,
...
GRANT EXECUTE ON FUNCTION public.get_public_bot_leaderboard(text, int) TO anon;
GRANT EXECUTE ON FUNCTION public.get_public_bot_leaderboard(text, int) TO authenticated;

COMMENT ON FUNCTION public.get_public_bot_leaderboard IS
  'Public HL bot win leaderboard — masked wallet, open/close times, profit. No emails or profile data.';
