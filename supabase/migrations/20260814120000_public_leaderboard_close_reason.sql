-- Public leaderboard: expose close_reason so Manual closes can be labeled.
...
BEGIN
  IF sort_key = 'recent' OR sort_key = 'recent_all' THEN
    ...
    SELECT
      ...
      nullif(trim(th.close_reason), '') AS close_reason
...
COMMENT ON FUNCTION public.get_public_bot_leaderboard IS
  'Public HL bot closes — includes close_reason for Manual / trail labels. Masked wallets only.';
